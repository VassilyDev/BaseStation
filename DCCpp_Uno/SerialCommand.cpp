...
void SerialCommand::process() {
  ...
  if (DOUBLE_SERIAL) {
    while (INTERFACE1.available() > 0) { // while there is data on the serial line
      c = INTERFACE1.read();
      if (c == '<')                 // start of new command
        sprintf(commandString, "");
      else if (c == '>')            // end of new command
  ...
  switch (com[0]) {
    ...
    case 'c':     // <c>
      ...
      */
      ...
      if (DOUBLE_SERIAL) {
        INTERFACE1.print("<a");
        INTERFACE1.print(int(mMonitor->current));
        INTERFACE1.print(">");
      }
      break;
    ...
    case 's':      // <s>
      ...
      */
      ...
      for (int i = 1; i <= MAX_MAIN_REGISTERS; i++) {
        ...
        if (DOUBLE_SERIAL) {
          if (mRegs->speedTable[i] == 0)
            continue;
          INTERFACE1.print("<T");
          INTERFACE1.print(i); INTERFACE1.print(" ");
          if (mRegs->speedTable[i] > 0) {
            INTERFACE1.print(mRegs->speedTable[i]);
            INTERFACE1.print(" 1>");
          } else {
            INTERFACE1.print(-mRegs->speedTable[i]);
            INTERFACE1.print(" 0>");
      ...
      INTERFACE.print(": ");
      if (DOUBLE_SERIAL) {
        INTERFACE1.print("<iDCC++ BASE STATION FOR ARDUINO ");
        INTERFACE1.print(ARDUINO_TYPE);
        INTERFACE1.print(" / ");
        INTERFACE1.print(MOTOR_SHIELD_NAME);
        INTERFACE1.print(": V-");
        INTERFACE1.print(VERSION);
        INTERFACE1.print(" / ");
        INTERFACE1.print(__DATE__);
        INTERFACE1.print(" ");
        INTERFACE1.print(__TIME__);
        INTERFACE1.print(">");

        INTERFACE1.print("<N");
        INTERFACE1.print(COMM_TYPE);
        INTERFACE1.print(": ");
      }
      ...
      if (DOUBLE_SERIAL) {
        INTERFACE1.print("SERIAL");
      }
#elif COMM_TYPE == 1
      INTERFACE.print(Ethernet.localIP());
      INTERFACE.print(">");
      if (DOUBLE_SERIAL) {
        INTERFACE1.print(Ethernet.localIP());
        INTERFACE1.print(">");
      }
#endif
    ...
    case 'E':     // <E>
      ...
      */
      ...
      if (DOUBLE_SERIAL) {
        INTERFACE1.print("<e ");
        INTERFACE1.print(EEStore::eeStore->data.nTurnouts);
        INTERFACE1.print(" ");
        INTERFACE1.print(EEStore::eeStore->data.nSensors);
        INTERFACE1.print(" ");
        INTERFACE1.print(EEStore::eeStore->data.nOutputs);
        INTERFACE1.print(">");
      }
      break;
    ...
    case 'F':     // <F>
      ...
      */
      ...
      if (DOUBLE_SERIAL) {
        INTERFACE1.print("<f");
        INTERFACE1.print((int) &v - (__brkval == 0 ? (int) &__heap_start : (int) __brkval));
        INTERFACE1.print(">");
      }
      break;
    ...
    case 'L':     // <L>
      ...
      */
      ...
      if (DOUBLE_SERIAL) {
        INTERFACE1.println("");
        for (Register *p = mRegs->reg; p <= mRegs->maxLoadedReg; p++) {
          INTERFACE1.print("M"); INTERFACE1.print((int)(p - mRegs->reg)); INTERFACE1.print(":\t");
          INTERFACE1.print((int)p); INTERFACE1.print("\t");
          INTERFACE1.print((int)p->activePacket); INTERFACE1.print("\t");
          INTERFACE1.print(p->activePacket->nBits); INTERFACE1.print("\t");
          for (int i = 0; i < 10; i++) {
            INTERFACE1.print(p->activePacket->buf[i], HEX); INTERFACE1.print("\t");
          }
          INTERFACE1.println("");
        }
        for (Register *p = pRegs->reg; p <= pRegs->maxLoadedReg; p++) {
          INTERFACE1.print("P"); INTERFACE1.print((int)(p - pRegs->reg)); INTERFACE1.print(":\t");
          INTERFACE1.print((int)p); INTERFACE1.print("\t");
          INTERFACE1.print((int)p->activePacket); INTERFACE1.print("\t");
          INTERFACE1.print(p->activePacket->nBits); INTERFACE1.print("\t");
          for (int i = 0; i < 10; i++) {
